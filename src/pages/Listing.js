  ...
  return (
    <main>
        ...
        effect="fade" modules={[EffectFade]} autoplay={{delay:3000}}>
            {listing.imgUrls.map((url, index)=>(
                <SwiperSlide key={index}>
                    <div className='w-full overflow-hidden h-[300px] relative' style={{background:`url(${listing.imgUrls[index]}) center no-repeat `,
        ...
        {shareLinkCopied && (
            ...
            z-10 p-2'>Link Copied</p>
        )}


        <div className='flex flex-col md:flex-row max-w-6xl 
        lg:mx-auto p-4 rounded-lg shadow-lg bg-white lg:space-x-5 mt-6
        '>
            <div className='w-full '>
                <p className='
                text-2xl font-bold mb-3 text-blue-900 
                '>{listing.name} - $ {(listing.offer ? listing.discountedPrice : listing.regularPrice).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 
                ...
                space-x-4 w-[75%] '>
                    ...
                    shadow-md '>{listing.type === "rent" ? "Rent" : "Sale"}</p>
                    {listing.offer && (
                        <p className='w-full max-w-[200px] bg-green-800 rounded-md
                        text-white p-1 text-center font-semibold shadow-md'>$ {listing.regularPrice - listing.discountedPrice} discount</p>
                    )}
                </div>
                    ...
                    font-medium text-sm uppercase rounded
